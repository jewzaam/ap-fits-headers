...
This script preserves FITS headers from file path key-value pairs
into .fits and .xisf files. It extracts key-value pairs encoded in
directory paths and filenames and inserts them as FITS header keywords,
but only if the header value doesn't already match the path value.
"""
...
def extract_key_value_pairs(path: Path) -> Dict[str, str]:
    ...
    """
    # Use ap-common's path parsing logic
    # (without normalization or special profile/object handling)
...
def read_fits_header(filepath: Path) -> fits.Header:
    ...
    try:
        ...
        with fits.open(str(filepath), mode="readonly") as hdul:
            # Return a copy so it remains valid after close
...
def preserve_headers(
    ...
    Args:
        ...
        include_headers: List of specific header keys to include
            (required, explicit inclusion only)
    ...
    for filepath in progress_iter(
        ...
        try:
            ...
            # Error already logged by read function, just track failure
            files_failed += 1
            continue
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Preserve FITS headers from file path"
            " key-value pairs into .fits and .xisf files"
        )
    )
    parser.add_argument(
        "root_dir",
